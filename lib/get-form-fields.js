...
const getFormFields = (form) => {
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    // adding an name based exception because textarea don't have type attributes so despite adding it to html it 'falls off' on dom render and throws an error below at 57
    if (e.getAttribute('name') === 'rsvp[notes]') {
      // console.log('in get form fields the e object is: ', e)
      e.setAttribute('type', 'text')
    }
    // this adds the above exception to all textarea's, html will return all caps tagName
